...
常有客户疑惑，为什么只有一个设备在线时，Portal 的在线用户统计数量却是 2 ？原因很简单，Portal 也同时是一个 [JavaScript](https://github.com/yunba/yunba-javascript-sdk) 客户端，可以向指定应用（[AppKey][3]）的指定 Topic 发布消息，因此会算在统计内。
...
## 利用云巴 Portal 发布消息
客户端集成 YunBa SDK 后，打开 Portal 上应用详情页面，可以向客户端 `subscribe` 的 [频道][5]（Topic）发布消息，客户端即可收到消息，如图所示:
...
- 点击 **更多选项** 可设置 [离线消息](http://yunba.io/docs2/yunba_offline_message) 保留时间（Time To Live）、[QoS][2] 值 和 Message ID（如果不填则由系统自动生成）
- 可设置 **APN JSON** 发送 APNs 消息，发送 APNs 消息的方法具体可参考 [如何通过云巴实现 APNs 推送](http://yunba.io/docs2/ios_apns_guide) 和 [APNs 参数设置](https://developer.apple.com/library/ios/documentation/NetworkingInternet/Conceptual/RemoteNotificationsPG/Chapters/TheNotificationPayload.html#//apple_ref/doc/uid/TP40008194-CH107-SW1)。
...
如图所示，可查看该应用（[AppKey][3]） 下的消息发布和送达情况。
...
点击 **频道用户列表**，可查看应用（[AppKey][3]） 下某个 [频道][5] 的收听用户别名列表。


[1]: http://yunba.io/docs2/portal
[2]: http://yunba.io/docs2/qos
